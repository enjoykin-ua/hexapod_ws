...
| `hexapod_sensors` | Python/URDF | **IMU real** (A5 Stufe 0): `imu_monitor` (`/imu/data`→roll/pitch, `/imu/monitor`, world→base_link-tf); `bno055_imu` (HW-IMU); Foot-Contact-Publisher. **Ph.7B:** `hexapod_camera` (`rpicam_node`) — OV5647 via `rpicam-vid` MJPEG → `CompressedImage` auf `/camera/image_raw/compressed`; `source:=rpicam\|test`, `camera_enable`. IMU-xacro in `hexapod_description` (`hexapod.imu.xacro`). |
| `hexapod_audio` | Python | **Block I Ph.7A — Audio.** `hexapod_audio`-Node spielt kurze mp3s auf dem Roboter-Speaker (MAX98357A) via `mpg123`: Auto-Sounds bei Bewegung (aus `/hexapod/audio_cue` vom gait_node) + manuelle Sounds (`/hexapod/play_sound`, App). Mute nur der Auto-Sounds via `sound_enable` → latched `/hexapod/sound_enabled`. `config/sound_map.yaml` + `sounds/*.mp3` (git-versioniert). Sim = log-only (`playback_enabled`). |
...
- **Show-Auswahl (Block I Ph.8 + Ph.10):** Param **`show_mode`** (`none|look_around|dancing|free_leg`) mit **eigenem Gate** (Show-Modi nur aus STANDING, `none` immer → garantierter Rückweg). `look_around` → `STATE_BODY_POSE`: Körper in 6 DOF über **fixen Füßen** (Sub `/cmd_body_pose`), per-Achse-Envelope + Greedy-Achsen-Clamp → **kein IKError/Freeze aus der Show**. **`free_leg` (Block I Ph.10)** → die B4-Kette `SHOW_ENTER`/`SHOW_ACTIVE`/`SHOW_EXIT`: Körper-Rückversatz + **Nase-hoch-Pitch**, die 4 hinteren Beine stützen, die 2 Vorderbeine folgen `/cmd_show`. Eintritt erzwingt die **tiefe Stance** (bester Kippwinkel; via `_pending_show_enter` nach dem Stance-Wechsel nachgeholt), CoG-Gate in Phase b rechnet **pitch-korrekt im Welt-Frame**. `dancing` = Platzhalter (akzeptiert, no-op). `show_mode` steht im `/hexapod/status` und wird bei Recovery serverseitig auf `none` zurückgesetzt. Contract §6c.
...
| `/cmd_show` | Float64MultiArray[6] | joy_to_twist → gait_node | **Free-Leg-Show** (Vorderbeine frei): `[l6_lat,l6_vert,l6_radial,l1_lat,l1_vert,l1_radial]`, **normiert −1..+1** (der Node skaliert mit `show_*_scale`), R1-gegated, nur in `SHOW_ACTIVE`; ohne frisches Topic (>`cmd_vel_timeout`) federn die Beine in die Neutral-Pose. **Ph.10:** Einstieg app-exklusiv über `show_mode='free_leg'` — der Controller-Toggle bleibt per `show_enabled: false` aus, die **Bedienung** hängt an `show_sticks_enabled` (an) |
