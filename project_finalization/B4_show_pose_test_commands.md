...
```bash
# Reihenfolge [leg6_lat, leg6_vert, leg1_lat, leg1_vert], Werte in [-1, 1].
...
ros2 topic pub -r 10 /cmd_show std_msgs/msg/Float64MultiArray "{data: [0.0, 1.0, 0.0, 1.0]}"
# Erwartet: beide Vorderbeine heben weiter (vertikal +). Ctrl-C → zurück in Neutral.

# Linkes Vorderbein (leg_6) SEITWÄRTS, rechtes (leg_1) RUNTER:
ros2 topic pub -r 10 /cmd_show std_msgs/msg/Float64MultiArray "{data: [1.0, 0.0, 0.0, -1.0]}"
...
#   oder den Körper berühren — falls ja, später show_lat_scale/vert_scale senken.
...
- **R1 loslassen / Stick zentrieren** → Vorderbeine fahren in die Neutral-Hoch-Pose zurück.

🟡 **Signs verifizieren:** Stick HOCH muss das Bein HEBEN. Falls invertiert →
`sign_show_vert` (bzw. `sign_show_lat`) in `ps4_usb.yaml`/`ps4_bt.yaml` auf `-1.0` setzen.
🟡 **Tuning:** zu große Auslenkung / Kollision → `show_lat_scale` / `show_vert_scale`
(Param am gait_node) konservativer:
```bash
