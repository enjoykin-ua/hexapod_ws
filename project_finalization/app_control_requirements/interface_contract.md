...
| **v0.14** | **`free_leg` ist implementiert** (Block I Phase 10) — aus dem Platzhalter wird die B4-Show: der Roboter stützt sich auf die **4 hinteren Beine**, die **2 Vorderbeine** folgen den Sticks (linker Stick = linkes Vorderbein, rechter = rechtes, Trigger = ausstrecken, **R1 = Dead-Man**). **Kein neues Interface** — gesetzt wird weiter über den Param `show_mode`, bedient über `/joy` → `/cmd_show`; **die App muss nichts ändern** (genau der Zweck des Platzhalters, [D-Show-5]). Zwei Verhaltens-Zusagen sind neu: (1) der Roboter wechselt beim Start **selbstständig in die tiefe Stance** (bester Kippwinkel) und **bleibt danach dort**; (2) der Show-Start dauert dadurch bis zu **~7 s** (Stance-Wechsel + 4 s Eintritt). `status.show_mode` meldet `free_leg` bereits **während** des Stance-Wechsels. `dancing` bleibt Platzhalter. §6c |
| **v0.14.1** (aktuell) | **Kein Interface-Change** — Werte-Update aus Block I Phase 11 („Längere Schritte"). Die dynamischen Caps `status.step_length_cap` liegen höher (tief **0.065** / mittel **0.085** / hoch **0.055**; `step_height_cap` unverändert 0.04/0.05/0.08 — der Fuß-Hub ist geometrisch ausgereizt), und der **Boot-Wert von `cycle_time` ist 3.4 s** statt 2.0 (längere Bodenzeit = längere Schritte bei gleicher Fahrgeschwindigkeit; die Tempo-Stufen liegen jetzt bei 4.0/3.6/3.4/1.7 s). **Die App muss nichts ändern:** die Slider klemmen weiterhin live auf `status.*_cap`, und die Manifest-Grenzen decken die neuen Werte ab — nur die `default`-Felder im `hmi_config_manifest.yaml` wurden nachgezogen (Always-On-Neustart beim Deploy). Nebenbei: `cmd_vel clamped` wird nur noch bei **deutlicher** Begrenzung als WARN gemeldet (vorher jede, alle 2 s) — die Alert-Liste bleibt im Normalbetrieb sauber. **Ein neuer Param im Config-Panel:** `time_from_start_factor` (`/gait_node`, „Bahn-Nachführung", 1.0…4.0, Default 2.0) — er bestimmt, wie viel der kommandierten Schrittweite die Beine real ausführen (Sim-Messung: bei 4.0 nur 66 %, bei 1.0 volle Amplitude). **Keine App-Änderung nötig** (das Panel rendert generisch aus dem Manifest); die Always-On-Schicht muss nach dem Deploy einmal neu starten. |
| **v0.13.2** | Drei Festlegungen aus der Phase-9-App-Integration (**ROS-Seite implementiert + unit-getestet**): (1) **`performed=True/False` in der `message` von `/hexapod_pi_shutdown` ist ein STABILER VERTRAGSTEXT** — die App unterscheidet daran „ausgeführt" von „geblockt"; der Wortlaut wird nicht mehr geändert (§2a). (2) **Freeze-Reject:** die Sequenz-Services (`sit_down`/`stand_up`/`shutdown`/`cycle_stance`/`show_toggle`) lehnen bei aktivem E-Stop **sofort mit Klartext-Grund** ab, statt `success=true` zu melden und nichts zu tun; `estop`/`recover`/`pi_shutdown` bleiben erreichbar ([D-Feld-9]). (3) **`gait_delay` ↔ App-Timeout:** der `gait_node` startet erst 12 s nach `bringup_start` — „Stack läuft" (`bringup_status`) ist **nicht** „bedienbar"; der belastbare Indikator ist der erste `/hexapod/status`-Tick (App-Timeout 40 s). |
...
{"state":"STANDING","stance_idx":1,"stance":"mittel","gait":"tripod",
 "safety_frozen":false,"tip":"none","step_height_cap":0.05,"step_length_cap":0.085,
 "show_mode":"none"}
```
| Feld | Typ | Werte |
|---|---|---|
...
**40 Params** (24 sichtbar + 16 Balance-Gains advanced).
...
| `free_leg` | **Free-Leg** | **implementiert (Phase 10)** — 4 hintere Beine stützen, die 2 Vorderbeine folgen den Sticks; der Körper lehnt sich dabei 5° zurück |
