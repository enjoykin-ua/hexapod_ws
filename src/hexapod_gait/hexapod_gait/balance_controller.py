...
v1-Gesetz pro Achse (Master D3): **Totband-PI + Slew-Rate-Limit + Anti-Windup**.
...
class BalanceController:
    ...
    def __init__(
        ...
        """
        Alle Winkel in rad, ``slew_max`` in rad/s, ``ki`` in 1/s.
    ...
    def update(
        ...
        ``dt <= 0`` (erster Tick / Zeitsprung): Ausgang unverändert halten
        (Slew-Step 0), kein Integrieren.
        """
        self._out_roll, self._i_roll, self._err_roll = self._update_axis(
            roll, dt, self._i_roll, self._out_roll,
        )
        self._out_pitch, self._i_pitch, self._err_pitch = self._update_axis(
            pitch, dt, self._i_pitch, self._out_pitch,
        )
        return (self._out_roll, self._out_pitch)
    ...
    def _update_axis(
        ...
        """Eine Achse: Totband-PI + Anti-Windup + Slew. Returns (out, I, error)."""
        error = -measured  # Soll = 0 (horizontal)
        ...
        raw = _clamp(p_term + integ, self._max)
